...
<head>
    ...
    <title>Edit Discount: {{ $discount->code }}</title>
    ...
    <div class="main-container">
        <main class="main-content">
            ...
            <div class="form-container">
                <form action="{{ route('admin.discounts.update', $discount->id) }}" method="POST" class="discount-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" id="type" class="form-input" required>
                            ...
                            <option value="percentage" {{ old('type', $discount->type) == 'percentage' ? 'selected' : '' }}>Percentage
                            </option>
                        </select>
                        @error('type')
                            <span class="error">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-with-suffix">
                            <input type="number" name="min_order_value" id="min_order_value"
                                value="{{ old('min_order_value', $discount->min_order_value) }}" placeholder="e.g. 100000" min="0"
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-with-suffix">
                            <input type="number" name="max_discount_amount" id="max_discount_amount"
                                value="{{ old('max_discount_amount', $discount->max_discount_amount) }}" placeholder="e.g. 50000" min="0"
                                step="0.01" class="form-input">
                            <span class="suffix">VND</span>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="datetime-local" name="expires_at" id="expires_at" value="{{ old('expires_at', $discount->expires_at ? \Carbon\Carbon::parse($discount->expires_at)->format('Y-m-d\TH:i') : '') }}"
                            class="form-input">
                        @error('expires_at')
                            <span class="error">{{ $message }}</span>
                        @enderror
